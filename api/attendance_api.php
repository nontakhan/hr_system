...
try {
    ...
    if ($method === 'POST') {
        ...
        $month = $_POST['month'] ?? '';
        if (!preg_match('/^\d{4}-\d{2}$/', $month)) sendJsonError('กรุณาเลือกเดือนที่ต้องการนำเข้า');
        if (!isset($_FILES['csv_file']) || $_FILES['csv_file']['error'] !== UPLOAD_ERR_OK) {
            sendJsonError('กรุณาเลือกไฟล์ CSV');
        }

        $result = importAttendanceCsv($mysqli, $_FILES['csv_file']['tmp_name'], $month, $_FILES['csv_file']['name']);
...
function importAttendanceCsv($mysqli, $filePath, $month, $sourceFile) {
    ...
    foreach ($rows as $row) {
        if (substr($row['work_date'], 0, 7) !== $month) {
            $skipped++;
            continue;
        }
...
function buildMonthlyAttendanceReport($mysqli, array $employee, $month) {
    ...
    for ($date = $start; $date <= $end; $date = $date->modify('+1 day')) {
        ...
        $status = attendanceEvaluateStatus($workDate, $record['check_in'], $record['check_out'], $shift, $holidays);
        $rows[] = [
            'work_date' => $workDate,
