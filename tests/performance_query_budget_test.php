...
$db->statements = [];
$month = buildMonthlyAttendanceReport($db, $employee, '2026-01');
$monthlyQueries = count($db->statements);
$db->statements = [];
$range = buildAttendanceReportRange($db, $employee, '2026-01', '2026-12');
if (count($db->statements) > $monthlyQueries) {
    fwrite(STDERR, 'Range must load datasets once: ' . count($db->statements) . ' vs ' . $monthlyQueries . "
");
    exit(1);
}
if (count($range) !== 365 || array_slice($range, 0, 31) !== $month) throw new RuntimeException('Month/range mismatch');
$db->statements = [];
fetchEmployeeRequestReportLeaveEvents($db, 1, '2026-01', 'Mon,Tue', []);
if (count($db->statements) !== 1) throw new RuntimeException('Timeline must reuse work days and holiday context');
$db->statements = [];
approvalBadgeFetchCounts($db, 'admin', 1, []);
if (count($db->statements) !== 3) throw new RuntimeException('Badges must use three queries');
echo "PASS constant range query count and grouped badges
";
